    ...
    try:
        ...
        if (pass_mark in range(0,121,20)) and (defer_mark in range(0,121,20)) and (fail_mark in range(0,121,20)):
            ...
            if total_marks != 120:
                ...
                if pass_mark == 120:
                    print("\nProgress")
                    global counter_1                        #global variable is to be called outside the function
                    counter_1 += 1

                elif pass_mark == 100:
                    print("\nProgress (module trailer)")
                    global counter_2                       #global variable is to be called outside the function 
                    counter_2 += 1

                elif 120>= exclude_cutoff > 40:
                    print("\nDo not Progress – module retriever")
                    global counter_3                       #global variable is to be called outside the function
                    counter_3 += 1
                    
                elif exclude_cutoff <= 40:
                    print("\nExclude")
                    global counter_4                       #global variable is to be called outside the function
                    counter_4 += 1
